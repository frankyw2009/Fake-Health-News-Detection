          ...
          for i in range(len(graph_data)):
              ...
              if graph_data[i]:
                ...
                if inputs['input_ids'].size(1) >510:
                  inputs = tokenizer(news, return_tensors="pt", max_length=510, truncation=True)

                outputs = bert(**inputs)
                root_news_feature = outputs.pooler_output
                # data[i]
                news_mean_list = [root_news_feature]
                ...
                for j in range(10):
                  ...
                  for k in range(10):
                    if 'tweet_id_' + str(k) in graph_data[i]['rts_' + str(j)]:
                      ...
                      if inputs['input_ids'].size(1) >510:
                        inputs = tokenizer(news_piece, return_tensors="pt", max_length=510, truncation=True)

                      outputs = bert(**inputs)
                      news_list.append(outputs.pooler_output)
                ...
                data = Data(x=node_feature, edge_index=edge_index, y=graph_data[i]['label'])
