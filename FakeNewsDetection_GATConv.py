...
#     plt.savefig(fname='graph19.jpg')
#     plt.show()
#
#
# from torch_geometric.utils import to_networkx
#
# data = train_gossipcop_dataset[9]
#
...
def visualize(h, color, epoch=None, loss=None, train_accuracy=None, test_accuracy=None):
    ...
    if torch.is_tensor(h):
        ...
        nx.draw_networkx(h, pos=nx.spring_layout(h, seed=42), with_labels=False,
                         node_color=color, cmap="Set2")
    plt.savefig(os.path.join('Figures', f'GAT_Conv_politifact_{epoch}_epoch_training_graph.jpg'),dpi=350)
    # plt.show()
...
plt.xlabel("epoch")
plt.ylabel("accuracy")
plt.title("Politifact Model Training Accuracy(GATConv)")
plt.legend()  # 加图例

# for x1, y1 in zip(x, y):
#     plt.text(x1, y1, str(y1), ha='center', va='bottom', fontsize=16)

plt.savefig(os.path.join('Figures','GATConv_politifact_training_graph.jpg'), dpi=350)
plt.show()

torch.save(model, 'GAT_politifact_model.pth')
